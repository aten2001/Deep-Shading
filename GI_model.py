...
class Shading_Model(torch.nn.Module):
    def __init__(self):
        ...
        self.down_0_conv = nn.Conv2d(in_channels=3,out_channels=16,kernel_size=3,stride=1,padding=1)
        ...
        self.up_4_to_3 = nn.Conv2d(in_channels=256,out_channels=256,kernel_size=4,stride=2,padding=1,groups=256)
        self.up_3_conv = nn.Conv2d(in_channels=256,out_channels=128,kernel_size=3,stride=1,padding=1,groups=8)
        self.up_3_to_2 = nn.Conv2d(in_channels=128,out_channels=128,kernel_size=4,stride=2,padding=1,groups=128)
        self.up_2_conv = nn.Conv2d(in_channels=128,out_channels=64,kernel_size=3,stride=1,padding=1,groups=4)
        self.up_2_to_1 = nn.Conv2d(in_channels=64,out_channels=64,kernel_size=4,stride=2,padding=1,groups=64)
        self.up_1_conv = nn.Conv2d(in_channels=64,out_channels=32,kernel_size=3,stride=1,padding=1,groups=2)
        self.up_1_to_0 = nn.Conv2d(in_channels=32,out_channels=32,kernel_size=4,stride=2,padding=1,groups=32)
        self.up_0_conv = nn.Conv2d(in_channels=32,out_channels=1,kernel_size=3,stride=1,padding=1)

    def forward(self, input_normal, input_position , input_ground_truth, input_light):
        
        image = torch.cat((input_normal, input_position, input_light), 1)
