...
class ShadingModel(torch.nn.Module):
    def __init__(self):
        ...
        self.ssim_loss = pytorch_ssim.SSIM(window_size = 4)
        ...
        self.up_5_to_4 = nn.ConvTranspose2d(in_channels=256,out_channels=256,kernel_size=4,stride=2,padding=1,groups=256)
        self.up_4_conv = nn.Conv2d(in_channels=384,out_channels=128,kernel_size=3,stride=1,padding=1,groups=16)

        self.up_4_to_3 = nn.ConvTranspose2d(in_channels=128,out_channels=128,kernel_size=4,stride=2,padding=1,groups=128)
        self.up_3_conv = nn.Conv2d(in_channels=192,out_channels=64,kernel_size=3,stride=1,padding=1,groups=8)

        self.up_3_to_2 = nn.ConvTranspose2d(in_channels=64,out_channels=64,kernel_size=4,stride=2,padding=1,groups=64)
        self.up_2_conv = nn.Conv2d(in_channels=96,out_channels=32,kernel_size=3,stride=1,padding=1,groups=4)
        
        self.up_2_to_1 = nn.ConvTranspose2d(in_channels=32,out_channels=32,kernel_size=4,stride=2,padding=1,groups=32)
        self.up_1_conv = nn.Conv2d(in_channels=48,out_channels=16,kernel_size=3,stride=1,padding=1,groups=2)

        self.up_1_to_0 = nn.ConvTranspose2d(in_channels=16,out_channels=16,kernel_size=4,stride=2,padding=1,groups=16)
        self.up_0_conv = nn.Conv2d(in_channels=24,out_channels=1,kernel_size=3,stride=1,padding=1)

    def forward(self, input_normal, input_position):
        ...
        image_0 = image.clone().detach().requires_grad_(True)
        image = self.pool(image)
        
        image = self.down_1_conv(image)
        image = self.act(image)
        #print("after 1", image.size())
        image_1 = image.clone().detach().requires_grad_(True)
        image = self.pool(image)
        
        image = self.down_2_conv(image)
        image = self.act(image)
        #print("after 2", image.size())
        image_2 = image.clone().detach().requires_grad_(True)
        image = self.pool(image)
        
        image = self.down_3_conv(image)
        image = self.act(image)
        #print("after 3", image.size())
        image_3 = image.clone().detach().requires_grad_(True)
        image = self.pool(image)
        
        image = self.down_4_conv(image)
        image = self.act(image)
        #print("after 4", image.size())
        image_4 = image.clone().detach().requires_grad_(True)
        image = self.pool(image)
